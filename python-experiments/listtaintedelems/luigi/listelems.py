...
# For a given design, runs a experiment to get which elements were tainted at which cycle.
...
def find_affected_components_worker(row_size, timestep):
    ret = []
    for row_id in range(row_size):
        if shared_taintmatrix_glob[row_size*timestep + row_id] != 0:
            ret.append(row_id)
    return ret
...
class ListElems(luigi.Task):
    ...
    def __init__(self, *args, **kwargs):
        super(ListElems, self).__init__(*args, **kwargs)
        self.experiment_name = "listelems-{}-{}-{}-{}-{}-{}".format(self.simulator, taintstr(self.taintbits), binarycrc(self.binary), self.simtime, self.design_name, self.instrumentation)
    ...
    def run(self):
        ...
        print("Extracting tainted signal ids...")
        with mp.Pool(processes=NUM_PROCESSES, initializer=init_worker, initargs=(shared_taintmatrix, )) as pool:
            tainted_signal_ids = pool.starmap(find_affected_components_worker, worker_params)
            pool.close()
            pool.join()
        print("Done: Extracting tainted signal ids.")

        print("tainted_signal_ids:", len(tainted_signal_ids))
        print("len worker_params :", len(list(worker_params)))
        print("len list range    :", len(list(range(0, len(taintmatrix[0]), vcd_timestep))))

        already_seen_signal_indices = set()
        for col_id in trange(0, len(taintmatrix[0]), vcd_timestep):
            already_displayed_col_id = False
            # print("Curr col id: {}, len: {}".format(col_id, len(tainted_signal_ids)))
            for signal_id in tainted_signal_ids[col_id//vcd_timestep]:
                if signal_id not in already_seen_signal_indices:
                    if not already_displayed_col_id:
                        already_displayed_col_id = True
                        tqdm.write("\nTime {}:".format(col_id//vcd_timestep))
                    already_seen_signal_indices.add(signal_id)
                    tqdm.write(signals_list[signal_id])


        # # Could be parallelized as well.
        # for col_id in trange(0, len(taintmatrix[0]), vcd_timestep):
        #     already_displayed_col_id = False
        #     for row_id, row in enumerate(taintmatrix):
        #         if row[col_id] != 0 and row_id not in already_seen_signal_indices:
        #             already_seen_signal_indices.add(row_id)
        #             if not already_displayed_col_id:
        #                 already_displayed_col_id = True
        #                 tqdm.write("\nTime {}:".format(col_id))
        #             tqdm.write(signals_list[row_id])
        print("Done: Finding the affected components.")
